...
class RealtimeGroqService(GroqService):
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        if GROQ_API_KEYS:
            ...
            for key in GROQ_API_KEYS:
                self._fast_llms.append(ChatGroq(
                    groq_api_key=key,
    ...
    def _extract_search_query(
        ...
        if not getattr(self, "_fast_llms", []):
            return question
        ...
        try:
            ...
            for i, llm in enumerate(self._fast_llms):
                try:
                    ...
                    logger.warning("[REALTIME] Query extraction with key #%d failed: %s", i + 1, e)
                    if i == len(self._fast_llms) - 1:
                        logger.warning("[REALTIME] All keys failed for query extraction, using raw question")
                        return question
            
    def search_tavily(self, query: str, num_results: int = 5) -> tuple:
