...
class TaskExecutor:
    ...
    def __init__(self, groq_service=None):
        ...
        self._http_client = httpx.Client(timeout=60, follow_redirects=True)
    ...
    def _generate_pollinations(self, prompt: str) -> Optional[tuple]:
        ...
        api_url = (
            ...
            f"?model=flux&width=1024&height=1024&nologo=true&private=true&enhance=true&safe=true"
        )
        logger.info("[TASK] Fetching Pollinations image: %s", api_url[:120])
        for attempt in range(3):
            try:
                resp = self._http_client.get(api_url)
